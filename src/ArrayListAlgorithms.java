...
public class ArrayListAlgorithms {
    ...
    public static int belowAverage(ArrayList<Integer> intList)
    {
        ...
        int num = 0;
        for (int num3: intList)
        {
            if (num3 < (total/ intList.size()))
            {
                num ++;
            }
        }
    ...
    public static void replaceWithCaps(ArrayList<String> wordList)
    {
        ...
        {
            if (wordList.get(index).substring(wordList.get(index).length() - 1, wordList.get(index).length()).equals("s"))
            {
                wordList.set(index, wordList.get(index).toUpperCase());
            }
        }
    }
    ...
    public static int indexOfMinimum(ArrayList<Integer> intList)
    {
        ...
        {
            if (lowest > intList.get(index))
            {
                value = index;
                lowest = intList.get(index);
            }
        }
        return value;
    }
    ...
    public static boolean areIdentical(ArrayList<Integer> numList1, ArrayList<Integer> numList2)
    {
        ...
        {
            for (int index = 0; index < numList1.size(); index ++)
            {
                if (numList1.get(index) != numList2.get(index))
                {
                    return false;
                }
            }
    ...
    public static void wackyVowels(ArrayList<String> wordList)
    {
        ...
        for (int index = 0; index < wordList.size(); index ++)
        {
            ...
            {
                wordList.add(index, wordList.get(index));
                index ++;
            }
        }
    }
    ...
    public static void removeDuplicates(ArrayList<Integer> intList)
    {
        ...
        {
            for (int index2 = index + 1; index2 < intList.size(); index2 ++)
            {
                if (intList.get(index) == intList.get(index2))
                {
    ...
    public static void duplicateUpperAfter(ArrayList<String> wordList)
    {
        for (int index = 0; index < wordList.size(); index ++)
        {
            wordList.add(index + 1, wordList.get(index).toUpperCase());
            index ++;
        }
    }
    ...
    {
        for (int index = 0; index < wordList.size() - index; index ++)
        {
    ...
    {
        int num = 0;
        for (int index = wordList.size() - 1; num < wordList.size(); index --)
        {
            if (wordList.get(index).indexOf("b") != -1)
            {
    ...
    public static ArrayList<Integer> modes(int[] numList)
    {
        ...
        {
            int times = 1;
            for (int index2 = index + 1; index2 < numList.length; index2 ++)
            {
                if (numList[index] == numList[index2])
                {
                    times++;
                }
            }
